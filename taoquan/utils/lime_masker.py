...
def lime_mask(verifier_model_path, claim, evidence):

    tokenizer = AutoTokenizer.from_pretrained(verifier_model_path)

    ds = FEVERClsDataset(tokenizer, [], 256)

    with torch.no_grad():
        model = AutoModelForSequenceClassification.from_pretrained(verifier_model_path).to(device)
        model.eval()
        ...
        try:
            ...
            print(masked_claim)
            return remove_space_before_punctuation(masked_claim)
